...
perform.test <- function(tissue.expr, geno.matrix, gene.set, gene.list, covar, start, end, weights.file.prefix, maf.cutoff = 0.05, perm = 10000) {
    ...
    require(RNOmni)
    require(data.table)
    p.values.vt <- as.data.frame(matrix(nrow = end-start+1, ncol = 5))
    p.values.lrt <- as.data.frame(matrix(nrow = end-start+1, ncol = 5))
    #p.values.skat <- as.data.frame(matrix(nrow = end-start+1, ncol = 5))
    colnames(p.values.vt) <- c("Gene_ID", "Distance", "MAFxCADD", "MAFxDistance", "DistancexCADDxMAF")
    colnames(p.values.lrt) <- c("Gene_ID", "Distance", "MAFxCADD", "MAFxDistance", "DistancexCADDxMAF")
    #colnames(p.values.skat) <- c("Gene_ID", "Distance", "MAFxCADD", "MAFxDistance", "DistancexCADDxMAF")
    ...
    weights.files <- paste(weights.file.prefix, c("dis", "maf", "cadd_scaled"), sep=".")
    ...
    for (i in start:end) {
         ...
         selected.weights.dis <- get.weights(weights[[1]], selected.snp, rare.var.idx, norm="20k", impute="zero")
         selected.weights.maf <- get.weights(weights[[2]], selected.snp, rare.var.idx)
         selected.weights.cadd <- get.weights(weights[[3]], selected.snp, rare.var.idx, norm="l2")

         i <- i-start+1

         p.values.vt[i, 1] = selected.gene
         p.values.vt[i, 2] = VT(E, G, perm = perm, ksi = selected.weights.dis)$sig.perm
         p.values.vt[i, 3] = VT(E, G, perm = perm, ksi = selected.weights.maf * selected.weights.cadd)$sig.perm
         p.values.vt[i, 4] = VT(E, G, perm = perm, ksi = selected.weights.maf * selected.weights.dis)$sig.perm
         p.values.vt[i, 5] = VT(E, G, perm = perm, ksi = selected.weights.dis * selected.weights.cadd * selected.weights.maf)$sig.perm

         p.values.lrt[i, 1] = selected.gene
         p.values.lrt[i, 2] = lrt_perm(expr = E, geno = G, causal_ratio = selected.weights.dis, perm = perm)
         p.values.lrt[i, 3] = lrt_perm(expr = E, geno = G, causal_ratio = selected.weights.maf * selected.weights.cadd, perm = perm)
         p.values.lrt[i, 4] = lrt_perm(expr = E, geno = G, causal_ratio = selected.weights.maf * selected.weights.dis, perm = perm)
         p.values.lrt[i, 5] = lrt_perm(expr = E, geno = G, causal_ratio = selected.weights.dis * selected.weights.cadd * selected.weights.maf, perm = perm)
         
         #p.values.skat[i, 1] = selected.gene
         #p.values.skat[i, 2] = SKAT_(E, G, covariates = NULL, weight = norm.l2(selected.weights.dis))$p.value
         #p.values.skat[i, 3] = SKAT_(E, G, covariates = NULL, weight = norm.l2(selected.weights.maf * selected.weights.cadd))$p.value
         #p.values.skat[i, 4] = SKAT_(E, G, covariates = NULL, weight = norm.l2(selected.weights.maf * selected.weights.dis))$p.value
         #p.values.skat[i, 5] = SKAT_(E, G, covariates = NULL, weight = norm.l2(selected.weights.dis * selected.weights.cadd * selected.weights.maf))$p.value
