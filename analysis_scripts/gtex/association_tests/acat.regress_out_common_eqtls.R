...
perform.test <- function(tissue.expr, geno.matrix, gene.set, gene.list, covar, start, end, weights.file.prefix, common.eqtl, common.geno.matrix, maf.cutoff = 0.05, perm = 100000, nodes=100) {
    require(stringr)
    require(RNOmni)
    require(data.table)
    require(doMC)
    ...
    for (i in start:end) {
         ...
         if (is.null(G) | is.null(common.snp.genotype)) {
            i <- i-start+1
            p.values.acat[i, 1] = selected.gene
            print("ACAT")
            print(p.values.acat[i, ])
